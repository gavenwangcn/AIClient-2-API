...
const translations = {
    'zh-CN': {
        ...
        'dashboard.routing.mcp': 'MCP 代理',
        'dashboard.routing.mcpProxyNote': '需已 OAuth；路径前缀与主模型无关时请加 /consensus-mcp-oauth 或使用请求头 model-provider: consensus-mcp-oauth。',
        'dashboard.routing.mcpSectionCurl': '经本服务 HTTP（curl，可在任意机器执行）',
        'dashboard.routing.mcpCurlRemoteHint': '从其它电脑访问时，将 URL 主机改为部署 AIClient 的地址（端口、路径不变）；API Key 与本服务设置一致。',
        'dashboard.routing.mcpSectionRemoteMcporter': '远端 mcporter（把 AIClient 当 MCP 入口，经代理访问 Consensus）',
        'dashboard.routing.mcpRemoteIntro': '在已安装 mcporter 的电脑或服务器终端执行（无需 Docker）。新建配置文件：url 指向本 AIClient 的 MCP Streamable 端点，headers 携带 Authorization Bearer（与本服务 API Key 一致）。Consensus 的 OAuth 只需在「运行 AIClient 并完成网页授权」的那台服务器上完成一次。',
        'dashboard.routing.mcpRemoteUrlHint': '下方 JSON 里的 base URL 默认使用当前浏览器打开的站点；从其它机器连时，请改成你能访问的 AIClient 公网/IP 与端口，路径保持 /consensus-mcp-oauth/v1/mcp 不变。',
        'dashboard.routing.mcpJsonExampleLabel': '配置文件示例（如 mcporter-via-aiclient.json）',
        'dashboard.routing.mcpMcporterListLabel': '列出工具（mcporter list）',
        'dashboard.routing.mcpMcporterCallLabel': '调用检索（mcporter call，selector 以实际 list 输出为准）',
        ...
        'dashboard.routing.nodeName.openblocklabs': 'OpenBlockLabs OB-1',
        ...
        'oauth.modal.callbackHost': '回调地址',
        'oauth.modal.callbackHostHint': '支持 localhost、IP 或 ngrok 完整地址。ngrok：在服务器运行 ngrok http 8085，将得到的 https://xxx.ngrok.io 填入，并在 Google Cloud Console 添加该地址为 redirect_uri',
        ...
        'oauth.ob1.selectMethod': '选择 OB-1 认证方式',
        'oauth.ob1.oauth': 'OB-1 设备授权',
        'oauth.ob1.oauthDesc': '通过 WorkOS 设备码完成 OpenBlockLabs 账号授权',
        'oauth.ob1.batchImport': '批量导入 OB-1 凭据',
        'oauth.ob1.batchImportDesc': '批量导入 refresh_token 或完整账号 JSON（refresh_token 可为空）',
        'oauth.ob1.tokensPlaceholder': 'CSV: email,password,access_token,refresh_token 或 JSON / accounts.json',
        'oauth.ob1.importInstructions': '支持 ob12api accounts.json、CSV(email,password,access_token,refresh_token)、{accounts:[...]}。有 refresh_token 可自动续期；为空则仅 access_token 模式。',
        'oauth.ob1.noTokens': '请输入有效的 OB-1 凭据',
        'oauth.ob1.importing': '正在导入...',
        'oauth.ob1.importingProgress': '正在处理: {current} / {total}',
        'oauth.ob1.importSuccess': '成功导入 {count} 个凭据',
        'oauth.ob1.importPartial': '部分导入成功: {success} 成功, {failed} 失败',
        'oauth.ob1.importError': '导入过程中出错',
        'oauth.ob1.startImport': '开始导入',
        'oauth.ob1.step1': '点击下方按钮在浏览器中打开授权页面',
        'oauth.ob1.step2': '使用 OpenBlockLabs / OB-1 账号完成设备码授权',
        'oauth.ob1.step3': '授权成功后凭据将自动保存到 configs/ob1/',
        'oauth.ob1.step4': '请在 {min} 分钟内完成授权',
        ...
        'config.oauth.customGemini.title': '自定义 Gemini/Antigravity OAuth 凭据（远程服务器必填）',
        'config.oauth.customGemini.desc': '服务部署在远程服务器时需自建 OAuth 客户端。方案一 ngrok：在服务器运行 ngrok http 8085，将得到的 https://xxx.ngrok.io 加入 Google 授权列表，OAuth 弹窗中填入该地址。方案二 SSH 隧道：本地执行 ssh -L 3000:localhost:3000 -L 8085:localhost:8085 用户@服务器IP，浏览器访问 localhost:3000。',
        'config.oauth.geminiClientId': 'Gemini CLI Client ID',
        'config.oauth.geminiClientSecret': 'Gemini CLI Client Secret',
        'config.oauth.antigravityClientId': 'Antigravity Client ID',
        'config.oauth.antigravityClientSecret': 'Antigravity Client Secret',
        'config.oauth.clientIdPlaceholder': '留空使用内置',
        'config.oauth.clientSecretPlaceholder': '留空使用内置',
        ...
        'guide.providers.gemini.desc': '通过 Google OAuth 认证访问 Gemini 模型，支持 gemini-3.5-flash、gemini-3.1-pro-preview 等模型',
    ...
    'en-US': {
        ...
        'dashboard.routing.mcp': 'MCP Proxy',
        'dashboard.routing.mcpProxyNote': 'Requires OAuth. If the primary model is not Consensus MCP, use the /consensus-mcp-oauth prefix or header model-provider: consensus-mcp-oauth.',
        'dashboard.routing.mcpSectionCurl': 'HTTP via this service (curl, from any machine)',
        'dashboard.routing.mcpCurlRemoteHint': 'From another host, change the URL host to your AIClient deployment; API key must match this server.',
        'dashboard.routing.mcpSectionRemoteMcporter': 'Remote mcporter (AIClient as MCP entry → Consensus via proxy)',
        'dashboard.routing.mcpRemoteIntro': 'Run on any machine where mcporter is installed (no Docker required). Create a config file: set url to this AIClient MCP endpoint and add Authorization Bearer headers (same API key as this service). Consensus OAuth is only needed once on the server that runs AIClient.',
        'dashboard.routing.mcpRemoteUrlHint': 'The JSON below uses the current site origin; from another machine, replace it with your reachable AIClient host/port; keep the path /consensus-mcp-oauth/v1/mcp.',
        'dashboard.routing.mcpJsonExampleLabel': 'Sample config (e.g. mcporter-via-aiclient.json)',
        'dashboard.routing.mcpMcporterListLabel': 'List tools (mcporter list)',
        'dashboard.routing.mcpMcporterCallLabel': 'Call search tool (adjust selector to match list output)',
        ...
        'oauth.modal.callbackHost': 'Callback Host',
        'oauth.modal.callbackHostHint': 'Supports localhost, IP, or full ngrok URL. For ngrok: run "ngrok http 8085" on server, enter the https://xxx.ngrok.io URL here, and add it as redirect_uri in Google Cloud Console',
        ...
        'oauth.ob1.selectMethod': 'Select OB-1 Auth Method',
        'oauth.ob1.oauth': 'OB-1 Device Authorization',
        'oauth.ob1.oauthDesc': 'Authorize via WorkOS device code for OpenBlockLabs',
        'oauth.ob1.batchImport': 'Batch Import OB-1 Credentials',
        'oauth.ob1.batchImportDesc': 'Import refresh_token strings or full account JSON (refresh_token may be empty)',
        'oauth.ob1.tokensPlaceholder': 'CSV: email,password,access_token,refresh_token or JSON / accounts.json',
        'oauth.ob1.importInstructions': 'Supports ob12api accounts.json, CSV(email,password,access_token,refresh_token), {accounts:[...]}. refresh_token enables auto-renewal; leave empty for access_token-only mode.',
        'oauth.ob1.noTokens': 'Please enter valid OB-1 credentials',
        'oauth.ob1.importing': 'Importing...',
        'oauth.ob1.importingProgress': 'Processing: {current} / {total}',
        'oauth.ob1.importSuccess': 'Successfully imported {count} credential(s)',
        'oauth.ob1.importPartial': 'Partial import: {success} succeeded, {failed} failed',
        'oauth.ob1.importError': 'Import failed',
        'oauth.ob1.startImport': 'Start Import',
        'oauth.ob1.step1': 'Click the button below to open the authorization page',
        'oauth.ob1.step2': 'Complete device authorization with your OpenBlockLabs / OB-1 account',
        'oauth.ob1.step3': 'Credentials will be saved to configs/ob1/ after success',
        'oauth.ob1.step4': 'Complete authorization within {min} minutes',
        ...
        'config.oauth.customGemini.title': 'Custom Gemini/Antigravity OAuth Credentials (Required for Remote Server)',
        'config.oauth.customGemini.desc': 'For remote server: create your own OAuth client. Option 1 ngrok: run "ngrok http 8085" on server, add the https://xxx.ngrok.io URL to Google authorized redirect URIs, enter it in OAuth modal. Option 2 SSH tunnel: run "ssh -L 3000:localhost:3000 -L 8085:localhost:8085 user@SERVER_IP" locally, access localhost:3000 in browser.',
        'config.oauth.geminiClientId': 'Gemini CLI Client ID',
        'config.oauth.geminiClientSecret': 'Gemini CLI Client Secret',
        'config.oauth.antigravityClientId': 'Antigravity Client ID',
        'config.oauth.antigravityClientSecret': 'Antigravity Client Secret',
        'config.oauth.clientIdPlaceholder': 'Leave empty to use built-in',
        'config.oauth.clientSecretPlaceholder': 'Leave empty to use built-in',
        ...
        'guide.providers.gemini.desc': 'Access Gemini models via Google OAuth, supporting gemini-3.5-flash, gemini-3.1-pro-preview and more',
